classdef (StrictDefaults) my_controllerPurePursuit_1< nav.algs.internal.PurePursuitBase
...
%
%   PP = controllerPurePursuit returns a pure pursuit system object, PP,
%   that computes linear and angular velocity inputs for a differential
%   drive robot using the Pure Pursuit algorithm.
%
%   PP = controllerPurePursuit('PropertyName', PropertyValue, ...) returns
...
%
%   controllerPurePursuit properties:
%
...
%
%   See also binaryOccupancyMap, mobileRobotPRM.
...
%#ok<*EMCA>
    ...
    methods
        ...
        %set.Waypoints Setter for Waypoints property
            ...
            obj.validateWaypoints(waypts, 'controllerPurePursuit', 'Waypoints');
            obj.Waypoints = waypts;
        end

        function obj = controllerPurePursuit(varargin)
        %controllerPurePursuit Constructor
            setProperties(obj,nargin,varargin{:},'Waypoints', ...
                              'DesiredLinearVelocity', 'MaxAngularVelocity', ...
                              'LookaheadDistance');
        end
        function last_wayp = ret_last_wp(obj)
            last_wayp = obj.LastPose;
        end
    end
    ...
    methods (Access = protected)
        ...
        %validateInputsImpl Validate inputs before setupImpl is called
            coder.internal.errorIf(isempty(obj.Waypoints), ...
                                   'nav:navalgs:purepursuit:EmptyWaypoints');
